...
const elements = document.querySelectorAll('[data-range]');
    ...
    document.addEventListener('DOMContentLoaded', function() {
	  ...
	  document.getElementById('submit-button').addEventListener('click', function() {
		var timeValue = document.getElementById('time-slider').value;
		var xhr = new XMLHttpRequest();
		xhr.open('POST', 'https://api.particle.io/v1/devices/420025000447313137363331/Online_Time');
		xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
		xhr.setRequestHeader('Authorization', 'Bearer 3d05d03c3074d2fc6961feaae13b2ee84b91fcc6');
		xhr.send('value=' + timeValue);
	});

	const submitButton = document.getElementById('submit-button');
submitButton.addEventListener('click', function() {
    const newTime = document.getElementById('time-slider').value;
    particleFunctionCall('setCountdownTime', newTime);
});

function particleFunctionCall(name, data) {
    const accessToken = '3d05d03c3074d2fc6961feaae13b2ee84b91fcc6';
    const deviceId = '420025000447313137363331';
    const url = `https://api.particle.io/v1/devices/${deviceId}/${name}`;
    ...
    const params = {
        ...
        body: `access_token=${accessToken}&args=${data}`
    };
